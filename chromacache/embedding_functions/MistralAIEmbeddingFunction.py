...
class MistralAIEmbeddingFunction(AbstractEmbeddingFunction):
    ...
    def split_big_batches(self, documents: Documents, batch_token_limit=12000) -> list[Documents]:
        ...
        Args:
            documents (Documents): a batch of documents
            batch_token_limit (int): the api's token limit for a single batch

        Returns:
            list[Documents]: a list of batches (list of list)
        """
        token_per_document = [len(self.tokenizer.encode(d)) for d in documents]
        subatches = []
        curr_batch = []
        curr_batch_size = 0
        for doc, doc_size in zip(documents, token_per_document):
